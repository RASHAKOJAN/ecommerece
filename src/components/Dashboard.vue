        ...
        <v-col cols="12" sm="12">
          <v-breadcrumbs :items="items" class="justify-center mt-n7" dark>
            <template v-slot:divider>
              <v-icon color="#7C92EF">mdi-chevron-right</v-icon>
            </template>
          </v-breadcrumbs>
        </v-col>
      </v-row> 
    ...
    <v-card tile class="mx-16 mt-n10 card1" color="white">
          ...
          <v-card height="250px" tile flat color="#3890D8">
            <v-row>
              <v-col cols="12" sm="4" class="pl-8 pt-5">
                ...
                <h6 class="white--text mt-8">MOUINTAIN BIKE</h6>
                <h3 class="white--text mt-8">Trek Speed Concept</h3>
                <v-btn rounded color="#4C9CDB" dark class="withoutupercase px-8 mt-3">
                  More Info
                </v-btn>
              </v-col>
              <v-col cols="12" sm="8">
                <v-img src="bicycle.png" max-height="260" max-width="400" contain class="mt-n2"></v-img>
              </v-col>
            </v-row>
          </v-card>
        </v-col>
          ...
          <v-card height="250px" tile flat color="#6857E1">
            <v-row>
              <v-col cols="12" sm="6" class="pl-8 pt-5">
                ...
                <h3 class="white--text mt-5">Sale Collections 2023</h3>
      ...
      <v-row>
        ...
        <v-col cols="3" class="py-0 pr-0 mt-n3">
          <v-card flat outlined tile>
            ...
            <v-list dense class="mt-n5">
              <v-list-item v-for="item in styles" :key="item.title">
                <v-list-item-content>
          ...
          <v-card flat outlined tile>
            ...
            <v-list dense class="mt-n5">
              <v-list-item v-for="item in brands" :key="item.title">
              <v-list-item-content>
                <v-list-item-title class="ml-6" v-text="item.title"></v-list-item-title>
              </v-list-item-content>
              <v-list-item-action>
                <v-checkbox color="primary" v-model="item.state"></v-checkbox>
              </v-list-item-action>
              </v-list-item>
            </v-list>
          </v-card>
          <v-card flat outlined tile>
            ...
            <v-chip-group column multiple>
              ...
              <v-chip filter color="#EFFB500" text-color="white" ></v-chip>
          ...
          <v-card flat outlined>
            ...
            <v-chip-group column multiple active-class="blue white--text" class="ml-2">
              <v-chip v-for="item in sizes" :key="item" :value="item" class="size">
                {{ item }}
              </v-chip>
            </v-chip-group>
          </v-card>
        ...
        <v-col cols="9" class="mt-n3">
          <v-row>
            <v-col cols="12" sm="4" v-for="(item, i) in clothes" :key="i" :class="item.class">
            <v-hover v-slot:default="{hover}">
              <v-card height="300" align="center" flat outlined tile>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="black" small dark>{{ item.sold }}</v-btn>
                </v-card-actions>
                <v-img :src=item.image width="200" height="200" contain></v-img>
                <v-card-text class="mt-n4">
                  <strong :class="hover ? 'red--text' : 'black--text'">{{ item.title }}</strong>
                </v-card-text>
                <v-expand-transition>
      ...
      <v-row>
        ...
        <v-col cols="12" sm="12">
          <h6 class = "text-center grey--text">
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Recusandae impedit voluptas animi quas iusto <br/> deserunt modi quidem at? Eligendi, eveniet nemo debitis tempora eius perferendis iste corrupti vitae dolorem asperiores!
          </h6>
        </v-col>
        <v-col cols="12" sm="12">
